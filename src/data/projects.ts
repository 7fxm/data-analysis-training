...
# - order_date: 订单日期
# - customer_id: 客户编号
# - region: 地区（华东/华北/华南/华中/西北/西南/东北）
...
# 数据说明：超市购物篮交易记录
# 字段含义：
# - transaction_id: 交易编号（一次购物）
# - product_name: 商品名称（牛奶/面包/鸡蛋/火腿）
# - quantity: 购买数量
# - price: 商品价格（元）
...
# 字段含义：
# - user_id: 用户编号
# - group: 实验分组（control=对照组，treatment=实验组）
# - conversion: 是否转化（1=转化，0=未转化）
# - revenue: 收入（元）
...
ORD009,100,50.0,5000.0
ORD010,2,200.0,400.0
ORD011,1,-50.0,-50.0
ORD012,1,1000.0,1000.0
ORD013,3,50.0,150.0
...
print(f"检测到异常值: {len(outliers_iqr)}条")
...
print(f"检测到异常值: {len(outliers_zscore)}条")
...
print("注意：C005在客户表中不存在，name为NaN")
...
print("2. LEFT JOIN: 保留左表所有记录")
print("3. RIGHT JOIN: 保留右表所有记录")
